...
impl ApnsService {
    ...
    pub fn init() -> Option<Arc<Self>> {
        let key_path = dirs::home_dir()?.join(".agentic-flowstate").join("apns-key.p8");

        if !key_path.exists() {
            tracing::warn!("[APNS] No .p8 key found at {:?} — push notifications disabled", key_path);
            return None;
        }
    ...
    pub async fn send(&self, device_token: &str, title: &str, body: &str, conversation_id: Option<&str>, agent_name: Option<&str>) -> Result<(), String> {
        ...
        let response = match self.client
            ...
            .header("apns-expiration", (SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() + 3600).to_string())
        ...
        if status.is_success() {
            ...
            tracing::warn!("[APNS] Push failed for {}: {}", &device_token[..std::cmp::min(8, device_token.len())], reason);
            Err(reason)
        }
    }
