...
use super::conversation_event_encoder::ConversationEventEncoder;
use crate::agents::codex_app_server::{
    app_server_generated_images_dir, spawn_codex_app_server, verify_codex_subscription_auth,
    CodexAppServerEvent, CodexAppServerOptions, CodexAppServerOutcome, CodexSandboxMode,
    CodexToolProfile, RunningCodexAppServer,
};
...
pub struct WorkerMessage {
    ...
    /// → [`WorkerMessage`] → [`ConversationEventEncoder::set_pending_client_id`]
...
pub struct ConversationWorker {
    ...
    encoder: ConversationEventEncoder,
...
impl ConversationWorker {
    pub fn new(
        ...
        let encoder = ConversationEventEncoder::new(&conversation_id);
        Self {
            db,
            conversation_id,
            manager,
    ...
    async fn process_runtime_message(
        ...
        if let Some(plan) = codex_coordinator_thread_plan.as_ref() {
            ...
            .await
            {
                let _ = agent_runners::finish_turn(&self.db, &runner_turn_id, "failed").await;
                fail_runtime_before_spawn(
                    self,
                    &assistant_message_id,
                    msg.message_metadata.as_deref(),
        ...
        let spawn_result = spawn_codex_app_server(CodexAppServerOptions {
        ...
        let mut runtime_tracking_failure: Option<String> = None;
                    ...
                    match maybe_event {
                        Some(ConversationRuntimeEvent::SessionStarted { session_id: tid }) => {
                            if let Some(existing_thread_id) = thread_id.as_deref() {
                                if existing_thread_id == tid {
                                    continue;
                                }
                                runtime_tracking_failure = Some(format!(
                                    "Codex changed thread identity during one turn: expected {existing_thread_id}, received {tid}"
                                ));
                                let _ = turn.terminate().await;
                                break;
                            }
                            ...
                            if is_codex_coordinator {
                                ...
                                .await
                                {
                                    ...
                                    .await;
                                    runtime_tracking_failure = Some(failure);
                                    let _ = turn.terminate().await;
                                    break;
                                }
                            }
        ...
        let outcome = match turn.wait().await {
            ...
            Err(e) => {
                ...
                let failure_message = runtime_tracking_failure
                    .clone()
                    .unwrap_or_else(|| format!("Agent runtime turn failed: {e}"));
                if !cancelled {
                    log_user_visible_runtime_failure(
                        &self.db,
                        &self.conversation_id,
        ...
        let completion = if cancelled {
        ...
        } else if let Some(message) = runtime_tracking_failure {
            ...
            RuntimeTurnCompletion::Failed(
                "Codex runtime completed without returning a thread id".to_string(),
            )
        };
        ...
        if is_codex_coordinator {
            ...
            let error_class = match &completion {
                RuntimeTurnCompletion::Failed(message)
                    if message.contains("thread") || message.contains("resume") =>
                {
            ...
            let coordinator_thread_id = match &completion {
                RuntimeTurnCompletion::Completed { session_id }
    ...
    /// Emit a StreamEvent: encode to Conversation events, persist, and broadcast.
    ///
    ...
    /// Conversation frames from the 8-event vocabulary (`message_start` /
    ...
    async fn emit_event(&mut self, event: &StreamEvent) {
        // Encode the StreamEvent into a sequence of Conversation events.
        ...
        let conversation_events = self.encoder.encode(event);
        for ae in conversation_events {
            let ae_type = ae.event_type();
            let ae_json = match serde_json::to_string(&ae) {
                Ok(j) => j,
                Err(e) => {
                    tracing::error!(
                        "[WORKER] Failed to serialize ConversationEvent for {}: {}",
                        self.conversation_id,
                        e
                    );
                    continue;
...
async fn maybe_insert_child_completion_status_to_parent(
    ...
    if suppress_parent_completion_relay(message_metadata) {
        ...
        agent_lifecycle::record_wake_suppressed(
            db,
            WakeKind::CompletionRelay,
            None,
            Some(child_conversation_id),
            None,
            "completion_relay_suppressed",
            None,
            None,
        )
        .await;
...
async fn insert_child_completion_status_to_parent(
    ...
    let report_id = batch_context
        .and_then(|context| context.report_id.clone())
        .unwrap_or_else(agent_lifecycle::new_report_id);
    ...
    if let Some(batch_context) = batch_context {
        if let Err(e) =
            enqueue_parent_coordinator_batch_wake_if_complete(db, &parent, batch_context).await
        {
            let error_text = e.to_string();
            agent_lifecycle::record_wake_missed(
                db,
                WakeKind::BatchCompletion,
                &parent.id,
                Some(&child.id),
                Some(&batch_context.batch_id),
                runner_capacity::classify_failure_message(&error_text),
                &safe_error_summary(&error_text),
            )
            .await;
            tracing::error!(
                "[WORKER] Failed to enqueue parent coordinator batch wake parent={} child={} child_batch_id={} status={}: {}",
                parent.id,
                child.id,
        ...
        agent_lifecycle::record_wake_suppressed(
            ...
            Some(&child.id),
            None,
            "completion_not_in_batch",
            None,
            None,
        )
        .await;
        tracing::info!(
            "[WORKER] Suppressed per-child coordinator wake parent={} child={} status={} reason=completion_not_in_batch",
            parent.id,
            child.id,
            terminal_status
        );
...
async fn enqueue_parent_coordinator_batch_wake_if_complete(
    ...
    let wait_duration_ms = wait_duration_ms_from_completed_children(&completed_children);
    let telemetry_context = child_batch_telemetry_context(batch_context);
    if completed_children.len() < batch_context.expected_count {
        ...
        agent_lifecycle::record_wake_delayed(
            db,
            &parent.id,
            &telemetry_context,
            completed_children.len(),
    ...
    if parent_coordinator_batch_wake_exists(db, &parent.id, &batch_context.batch_id).await? {
        ...
        agent_lifecycle::record_wake_deduped(
            db,
            &parent.id,
            &telemetry_context,
            "existing_wake_message_or_job",
        )
        .await;
        return Ok(());
    }

    agent_lifecycle::record_child_batch_completed(
        db,
        &parent.id,
        &telemetry_context,
        completed_children.len(),
    ...
    if enqueued.outcome == conversation_turn_jobs::ConversationTurnJobEnqueueOutcome::Merged {
        agent_lifecycle::record_wake_deduped(
            db,
            &parent.id,
            &telemetry_context,
        ...
        agent_lifecycle::record_wake_fired(
            ...
            Some(&batch_context.batch_id),
            &enqueued.id,
            batch_context.report_id.as_deref(),
            Some(completed_children.len()),
            wait_duration_ms,
        )
        .await;
    }
...
fn wait_duration_ms_from_completed_children(
    completed_children: &[CompletedChildCompletion],
) -> Option<i64> {
    let oldest_status_created_at = completed_children
        .iter()
        .filter_map(|child| child.status_created_at)
        .min()?;
    Some(
        Utc::now()
            .timestamp()
            .saturating_sub(oldest_status_created_at)
            .max(0)
            * 1000,
    )
}

fn child_batch_telemetry_context(
    context: &ChildBatchContext,
) -> agent_lifecycle::ChildBatchTelemetryContext {
    agent_lifecycle::ChildBatchTelemetryContext {
...
// =============================================================================
//
// These tests exercise the Conversation 8-event persistence pipeline. They
// confirm:
//
//   1. Encoder → `insert_conversation_event` → `get_events` round-trips
//      produce ONLY conversation protocol event_types — no the runtime adapter
...
//      the canonical Conversation frame ordering when replayed off the DB.
//   3. Every persisted `event_data` payload parses as valid Conversation JSON
//      whose `type` field matches the row's `event_type`.
//
// If a non-Conversation discriminant ever regressed back into the emit
// pipeline these assertions would detect it — the replayed event_types
// would include a non-Conversation tag, or the row counts would diverge
// from encoder output.
#[cfg(test)]
mod streaming_persistence_tests {
    use super::*;
    use crate::agents::conversation_events::ALL_EVENT_TYPES;
    use serde_json::json;
    ...
    /// These are the canonical 8 Conversation streaming event type tags, as
    /// exported by [`ALL_EVENT_TYPES`]. If a runtime adapter discriminant ever
    ...
    async fn parent_wake_test_pool(saturate_queue: bool) -> SqlitePool {
        ...
        sqlx::query(
            ...
            CREATE TABLE conversations (
                ...
                child_conversation_count INTEGER,
    ...
    async fn coordinator_wake_job_count_for_batch(pool: &SqlitePool, batch_id: &str) -> i64 {
        sqlx::query_scalar(
            r#"
            SELECT COUNT(*)
            FROM conversation_turn_jobs
        ...
        .fetch_one(pool)
        .await
        .expect("count coordinator wake jobs for batch")
    }

    async fn child_completion_card_count(pool: &SqlitePool, batch_id: Option<&str>) -> i64 {
        match batch_id {
            ...
            .fetch_one(pool)
            .await
            .expect("count child completion cards for batch"),
            None => sqlx::query_scalar(
                ...
                WHERE metadata IS NOT NULL
                  AND json_valid(metadata)
                  AND json_extract(metadata, '$.orchestration') = 'child_completion_status'
                  AND json_extract(metadata, '$.child_batch_id') IS NULL
                "#,
            )
            .fetch_one(pool)
            .await
            .expect("count unbatched child completion cards"),
        }
    }
    ...
    async fn seed_codex_coordinator_parent_with_children(pool: &SqlitePool, child_ids: &[&str]) {
        sqlx::query(
            ...
            INSERT INTO conversations (
                id, user_id, organization, agent, conversation_type,
        ...
        for (index, child_id) in child_ids.iter().enumerate() {
            sqlx::query(
                ...
                INSERT INTO conversations (
                    id, user_id, organization, agent, conversation_type,
                    parent_conversation_id, conversation_role, child_sort_order, title,
                    started_at, updated_at, status, message_count, last_event_index
                ) VALUES (
                    ?, 'alex', 'agentic-flowstate', 'code-execution', 'implementation',
                    'parent-1', 'sub_agent', ?, ?,
                    '2026-07-14T00:00:00Z', '2026-07-14T00:00:00Z', 'open', 0, -1
                )
                "#,
            ...
            .bind(child_id)
            .bind(index as i64)
            .bind(format!("Worker {}", index + 1))
            .execute(pool)
            .await
            .expect("seed child conversation");
        }
    }

    fn child_initial_turn_metadata(batch_id: &str, batch_size: usize, index: usize) -> String {
        serde_json::json!({
            "origin": "agent_orchestrated",
            "orchestrated_by": "api",
            ...
            "child_batch_index": index,
            "report_id": format!("af-report-{batch_id}"),
        })
        .to_string()
    }
    ...
    async fn run_turn_and_replay(
        ...
        let mut encoder = ConversationEventEncoder::new(conversation_id);
        encoder.set_pending_message_id("assistant-test-message");
        for ev in events {
            for ae in encoder.encode(ev) {
                let ae_type = ae.event_type();
                let ae_json = serde_json::to_string(&ae).expect("serialize conversation event");
    ...
    async fn unbatched_child_completion_records_card_without_coordinator_wake() {
        ...
        .await
        .expect("record unbatched child completion");

        assert_eq!(child_completion_card_count(&pool, None).await, 1);
    ...
    async fn three_child_mixed_terminal_batch_wakes_once_after_last_completion() {
        ...
        let terminal_children = [
            ...
            ("child-2", "failed", Some("worker failed")),
            ("child-3", "cancelled", None),
        ];
        for (index, (child_id, terminal_status, error_message)) in
            terminal_children.iter().enumerate()
        {
            let metadata = child_initial_turn_metadata(batch_id, 3, index + 1);
            maybe_insert_child_completion_status_to_parent(
                &pool,
                child_id,
                &format!("assistant-{}", index + 1),
            ...
            .await
            .expect("record batched child completion");

            assert_eq!(
                child_completion_card_count(&pool, Some(batch_id)).await,
                (index + 1) as i64
            );
            assert_eq!(
                coordinator_wake_job_count_for_batch(&pool, batch_id).await,
                i64::from(index == terminal_children.len() - 1),
                "batch must wake only after its final terminal child"
            );
        }

        let wake_metadata: String = sqlx::query_scalar(
            r#"
            SELECT message_metadata
            FROM conversation_turn_jobs
            WHERE json_extract(message_metadata, '$.child_batch_id') = ?
            "#,
        )
        .bind(batch_id)
        .fetch_one(&pool)
        .await
        .expect("load aggregate wake metadata");
        let wake_metadata: serde_json::Value =
            serde_json::from_str(&wake_metadata).expect("parse aggregate wake metadata");
        assert_eq!(wake_metadata["child_completion_count"], 3);
        assert_eq!(
            wake_metadata["child_conversations"]
                .as_array()
                .expect("child completion list")
                .iter()
                .map(|child| child["child_terminal_status"]
                    .as_str()
    ...
    #[tokio::test]
    async fn overlapping_batches_wake_once_each_on_independent_completion() {
        let pool = parent_wake_test_pool(false).await;
        ...
        for (child_id, assistant_id, batch_id, index) in [
            ...
            ("child-b1", "assistant-b1", batch_b, 1),
            ("child-a2", "assistant-a2", batch_a, 2),
        ] {
            let metadata = child_initial_turn_metadata(batch_id, 2, index);
            maybe_insert_child_completion_status_to_parent(
                &pool,
                child_id,
                assistant_id,
            ...
            .await
            .expect("record overlapping batch completion");
        }

        assert_eq!(
            coordinator_wake_job_count_for_batch(&pool, batch_a).await,
            1
        );
        assert_eq!(
            coordinator_wake_job_count_for_batch(&pool, batch_b).await,
            0
        );

        let metadata = child_initial_turn_metadata(batch_b, 2, 2);
        maybe_insert_child_completion_status_to_parent(
            &pool,
            "child-b2",
            "assistant-b2",
        ...
        .await
        .expect("finish second overlapping batch");
        ...
        assert_eq!(coordinator_wake_job_count(&pool).await, 2);
    }

    fn assert_all_conversation(types: &[String]) {
        for t in types {
            assert!(
    ...
    /// Canonical Conversation frame sequence: message_start, content_block_start(text),
    /// content_block_delta(text_delta), content_block_stop, message_delta, message_stop.
    #[tokio::test]
    async fn text_only_turn_persists_conversation_frames_only() {
    ...
    #[tokio::test]
    async fn tool_use_turn_persists_conversation_frames_only() {
        let pool = fresh_test_pool().await;
        let (types, values) = run_turn_and_replay(
            &pool,
            "c-t49352bf5",
            &[
                ...
                StreamEvent::ToolUse {
                    id: "tool-xyz".into(),
                    name: "search".into(),
                    input: json!({"q": "conversation streaming"}),
                },
                StreamEvent::Text {
                    content: "Done.".into(),
                },
        ...
        assert_eq!(partial, json!({"q": "conversation streaming"}));
    }
    ...
    /// takes over.
    #[tokio::test]
    async fn thinking_turn_persists_conversation_frames_only() {
        let pool = fresh_test_pool().await;
        let (types, values) = run_turn_and_replay(
            &pool,
            "c-t49352bf5",
    ...
    /// Silencer: confirm that the runtime adapter router/replay tags are NOT
    ...
    /// from the canonical `ALL_EVENT_TYPES` export in `conversation_events`.
    ...
    #[test]
    fn allow_list_matches_conversation_event_surface() {
        assert_eq!(
            ALLOWED_EVENT_TYPES, ALL_EVENT_TYPES,
            "scope-cut allow-list drifted from canonical ConversationEvent surface"
        );
    }
    ...
    fn child_completion_status_message_carries_summary_and_open_link() {
        ...
        assert!(message.contains("Report ID: `af-report-test`"));
    ...
    fn parent_coordinator_batch_wake_metadata_marks_aggregate_review_request() {
        ...
        let completed_children = vec![
            CompletedChildCompletion {
                ...
                status_created_at: Some(1_783_310_400),
            },
            CompletedChildCompletion {
                ...
                status_created_at: Some(1_783_310_460),
            },
        ];
