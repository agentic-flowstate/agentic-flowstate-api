...
async fn prepare_transcript(raw_text: &str) -> Result<String, (StatusCode, String)> {
    ...
    let system_prompt = load_prompt("tts-transcript", vars)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to load tts-transcript prompt: {}", e)))?;
    ...
    let result = run_oneshot(
        ...
        &working_dir,
        raw_text,
    ).await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Transcript preparation failed: {}", e)))?;
...
pub async fn text_to_speech(
    ...
    if req.text.len() > MAX_INPUT_LENGTH {
        return Err((StatusCode::BAD_REQUEST,
            format!("Text exceeds maximum length of {} characters", MAX_INPUT_LENGTH)));
    }
    ...
    if !VALID_FORMATS.contains(&response_format.as_str()) {
        return Err((StatusCode::BAD_REQUEST,
            format!("Invalid format. Must be one of: {}", VALID_FORMATS.join(", "))));
    }

    // Step 1: Clean raw text into a speakable transcript
    tracing::info!("Preparing transcript from {} chars of raw text", req.text.len());
    let clean_text = prepare_transcript(&req.text).await?;
    tracing::info!("Transcript prepared: {} chars\n---BEGIN TRANSCRIPT---\n{}\n---END TRANSCRIPT---", clean_text.len(), clean_text);
